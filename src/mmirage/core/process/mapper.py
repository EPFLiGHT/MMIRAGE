...
from mmirage.core.process.variables import BaseVar, InputVar, OutputVar, VariableEnvironment
...
class MMIRAGEMapper:
    ...
    def __init__(
        ...
        Args:
            ...
            image_asset_manager: Optional canonical resolver for image inputs.
        """
    ...
    def validate_vars(self) -> bool:
        ...
        for output_var in self.output_vars:
            if not output_var.is_computable(vars):
                ...
                logger.info(
                    f"Variable {output_var.name} not computable given current context: {context}"
                )
                return False
